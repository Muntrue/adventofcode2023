...
7pqrstsixteen";
        ...
        foreach ($lines as $line) {
            $line = $this->getDigitsFromWords($line);
            $onlyNumbers = preg_replace('/\D/', '', $line);

            $firstNumber = substr($onlyNumbers, 0, 1);
            $secondNumber = substr($onlyNumbers, -1, 1);
            $finalNumber = $firstNumber . "" . $secondNumber;

            $total = $total + (int)$finalNumber;
        }

        dd($total);
    }

    function getDigitsFromWords(string $line): string
    {
        $wordsToNumbers = [
            'one' => 1,
            'two' => 2,
        ...
        foreach ($wordsToNumbers as $word => $number) {
            if (str_contains($line, $word)) {
                ...
                foreach($this->getAllPositionsOfWord($word, $line) as $start){
                    $positions[] = collect([
                        'word' => $word,
                        'value' => $number,
                        'start' => $start,
        ...
        if (count($positions) > 0) {
            ...
            $positions = $positions->sortByDesc('start');

            $explodedString = str_split($line);
            $positions->each(function ($x) use (&$line, &$explodedString) {

                for ($i = $x['start']; $i < $x['end']; $i++) {
                    unset($explodedString[$i]);
                    $explodedString[$i] = "";
                }
                $explodedString[$x['start'] + 1] = $x['value'];
                ksort($explodedString);
            });

            ksort($explodedString);

            $line = implode("", $explodedString);
    ...
    function getAllPositionsOfWord(string $word, string $line, $startPositions = [], $offset = 0){
        $position = strpos($line, $word);
        $startPositions[] = $position + $offset;

        $subStr = substr($line, $position + strlen($word), strlen($line));

        if(str_contains($subStr, $word)){
            $startPositions =  $this->getAllPositionsOfWord($word, $subStr, $startPositions, $position + strlen($word));
        }
