...
hands.forEach((hand) => {
    ...
    let keyWithHighestPair = '0';
    let highestPairValue = 0;
    Object.keys(pairs).forEach((key) => {
        if(key === 'J') return;

        if(pairs[key].amount >= highestPairValue){

            const value = getValue(key);
            const keyValue = getValue(keyWithHighestPair);

            if(pairs[key].amount > highestPairValue || value > keyValue){
                keyWithHighestPair = key;
                highestPairValue = pairs[key].amount;
            }
        }
    });

    let usedJoker = false;
    if(keyWithHighestPair !== '0'){
        usedJoker = true;
        pairs[keyWithHighestPair].amount = pairs[keyWithHighestPair].amount + (pairs['J']?.amount || 0);
    }


    Object.keys(pairs).forEach((key) => {
        if(key === 'J' && usedJoker) return;
...
function sortByFirstCharacter(arr, property) {
    ...
    const customSort = (a, b) => {
        ...
        for (let i = 0; i < Math.min(aValue.length, bValue.length); i++) {
            const charA = getValue(aValue[i]);
            const charB = getValue(bValue[i]);
...
function getValue(input){
    return (Object.keys(cardScores).includes(input) ? cardScores[input] : parseInt(input));
}